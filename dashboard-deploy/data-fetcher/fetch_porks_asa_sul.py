#!/usr/bin/env python3
"""
Coleta dados para o dashboard de Porks Asa Sul.
Uso: python3 fetch_porks_asa_sul.py --start 2026-04-01 --end 2026-04-30
"""
...
OUT_DIR      = Path(__file__).parent.parent / "pages/porks-asa-sul"
META_ACCOUNT = "act_2420587051522130"
...
def extract_result(obj, goal, actions, spend):
    ...
    if goal == "REPLIES":
        ...
        # prioridade: conversão personalizada de reservas > lead de formulário > lead genérico
        ...
        # para qualquer objetivo, se tiver conversão personalizada, priorizar como Reservas
        custom_keys = [k for k in action_map if k.startswith("offsite_conversion.custom.")]
        if custom_keys:
            key, label = custom_keys[0], "Reservas"
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Coleta dados para Porks Asa Sul")
    ...
    data = {"cliente":"Porks Asa Sul","periodo":{"start":start,"end":end},"gerado_em":datetime.now().isoformat(),
        "consolidado":None,
        "meta":meta,
        "google":None,
        "ga4":None,
